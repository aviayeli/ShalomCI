...
- **שורת מדדי סיכום:** `summarize_risk()` (פונקציה טהורה ב-`table_rows.py`) + `render_summary_metrics()` (ב-`ui_helpers.py`) - סה"כ רכיבים / סיכון קריטי / אזהרה / תקינים כארבעה `st.metric` מעל סרגל הסינון.
