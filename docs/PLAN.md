...
- **RTL וטיפוגרפיה - שתי שכבות CSS:** כיוון RTL גלובלי, גופן נקי, גודל גופן מוגדל מעט (1.05rem) ואיסור italics מוגדרים גם ב-`RTL_CSS` הגלובלי של `app.py` (לכלל האפליקציה) וגם ב-`table_render._TABLE_STYLES` הממוקד ל-Styler (`direction: rtl`, `sticky` header, אותה טיפוגרפיה) - הכפילות מכוונת: ה-CSS של ה-Styler עצמו (המקובע אוטומטית לסלקטור ה-id הייחודי של הטבלה על ידי pandas) הוא זה שבאמת שולט ברינדור הטבלה, ללא תלות ב-CSS הגלובלי.
