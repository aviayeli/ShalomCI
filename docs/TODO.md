...
- [x] **תפריט נגישות Enable.co.il:** `src/gui/accessibility_widget.py` (מודול ייעודי חדש, בהתאם לדפוס הפיצול הקיים ב-`table_rows.py`/`table_controls.py`/`table_render.py`) - `inject_accessibility_widget()` מזריק אלמנט `<script>` ל-`window.parent.document.head` (לא ל-DOM המבודד של ה-iframe של `components.html` עצמו), כולל בדיקת `getElementById` למניעת הזרקה כפולה בכל rerun של Streamlit. נקרא מתוך `app.py::main()` מיד אחרי `st.set_page_config`.
- [x] **TDD לתפריט הנגישות:** `tests/unit/test_accessibility_widget.py` - מוודא את כתובת הסקריפט, היעד (`window.parent`), מנגנון מניעת הכפילות, וממדי ה-iframe (0x0, בלתי נראה).
...
## פתוח - ממתין לתוצאות ריצה אמיתית (לא חוסם, אך דורש מעקב)
- **תיקון שאילתת ה-GraphQL של Octopart:** smoke test אמיתי (Phase 11) חשף ששאילתת `supSearch` הנוכחית מוחזרת עם 400 Bad Request מול Nexar בפועל - כנראה אי-התאמה בין הסכימה שהונחה (מתועוד ציבורי) לסכימה האמיתית. התיקון הנוכחי (Phase 11) פותר את הסימפטום הקריטי (חסימת ה-UI) ואת החשיפה של הודעת השגיאה המדויקת בלוג, אך **לא** את שגיאת השאילתה עצמה - עדיין ממתין לגוף השגיאה המדויק מהלוג כדי לתקן את שמות השדות ב-`_PART_QUERY`.

---
## סטטוס פרויקט: שלב הפיתוח הושלם (Development Phase Complete)
כל שלבי הפיתוח שהוגדרו במסמך זה (Phase 1 עד Phase 11) הושלמו ואומתו בבדיקות, כולל שלב הליטוש הסופי (ציון סיכון מצטבר עם הסבר, מיון דינמי, ואריזת Desktop), אינטגרציית DigiKey ו-Octopart המלאות, ותיקון תקלת ה-Retry הקריטית + תפריט הנגישות. המערכת כעת multi-provider אמיתית (Mouser + DigiKey + Octopart) כפי שתוכנן במקור ב-PRD סעיף 4.2, וכבר אינה חסינה מפני חסימת UI על שגיאות 4xx מספק כלשהו. הפריט הפתוח היחיד שדורש מעקב (תיקון שדות שאילתת Octopart בפועל, ראו לעיל) אינו חוסם - המערכת נופלת בחזרה בבטחה לברירות מחדל בעברית כל עוד הוא לא תוקן, בכפוף לאותה מתודולוגיית עבודה (PRD → PLAN → TODO → קוד) המתוארת ב-`docs/CLAUDE.md`.