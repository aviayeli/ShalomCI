...
- [x] **מעבר ל-`st.dataframe` הטבעי:** `table_render.py` נכתב מחדש - הוסרה לחלוטין גישת ה-iframe/`pandas.Styler.to_html` הקודמת, לטובת `st.dataframe` עם `column_config`. נמנע מ-timeouts ב-WebSocket וכשלי רינדור iframe; sticky header ותגיות ARIA מובנים ב-Streamlit ללא צורך בפתרונות עוקפים.
- [x] **מחזור חיים/ציון סיכון פעם אחת בלבד:** הוסרו לחלוטין השדות `digikey_lifecycle`/`octopart_lifecycle` (מ-`digikey_api.py`/`octopart_api.py`) ועמודות "מחזור חיים (DigiKey/Octopart)" הכפולות מה-GUI - Mouser הוא המקור היחיד, מוצג פעם אחת בעמודות "סטטוס"/"ציון סיכון".
- [x] **מחיר/מלאי כמספרים גולמיים (לא טקסט מפורמט):** `MouserClient`/`DigiKeyClient`/`OctopartClient.parse_extra_fields` מחזירים כעת `{vendor}_price_value`/`{vendor}_stock_qty` כ-`float`/`None` (לא מחרוזות "Mouser: 24,755 במלאי"). `extract_number()` חדש ב-`src/shared/translations.py` משותף לחילוץ מספר ממחרוזת מפורמטת של ספק (Mouser); DigiKey/Octopart כבר קיבלו את הערך הגולמי מה-API. `format_inventory()` הוסר (הפך ללא בשימוש).
- [x] **עמודות מחיר/מלאי מקובצות לפי ספק:** `table_rows.py::vendor_price_stock_columns()` בונה "מחיר (₪) - {ספק}"/"מלאי זמין - {ספק}" לכל אחד משלושת הספקים. **הערה טכנית חשובה:** נבדק ואומת ש-`st.dataframe` (Streamlit 1.58) **אינו** תומך ב-`pandas.MultiIndex` עבור עמודות יחד עם `column_config` - ניסיון שילוב ביניהם קורס (`TypeError: keys must be str, int, float, bool or None, not tuple`) כי `column_config` דורש מפתחות מחרוזת. נבחרו עמודות שטוחות עם שם קבוצה+ספק מוטמע בשם (לא כותרת מקובצת/משורשרת אמיתית), כדי לא לוותר על `NumberColumn`/`ProgressColumn` - שהיה נחשב לחשוב יותר בפועל.
- [x] **"ספק מומלץ":** `table_rows.py::recommended_vendor()` (פונקציה טהורה, נבדקת) - מעדיף את הספק הזול ביותר מבין אלו שדיווחו מחיר; נופל בחזרה לספק עם המלאי הגבוה ביותר אם אף ספק לא דיווח מחיר; `None` (חוסר נתון) לעולם לא "מנצח" ערך אמיתי (כולל 0). עמודה ראשונה בטבלה.
- [x] **`st.column_config`:** `ProgressColumn` לציון סיכון (0-5) ולמלאי (0-10,000, קנה מידה בלבד), `NumberColumn` עם פורמט `"₪ %.2f"` למחיר - נבדק אמפירית מול Streamlit האמיתי (לא רק תיעוד) שהתחביר נתמך.
- [x] **RTL/טיפוגרפיה:** `RTL_CSS` ב-`app.py` הורחב - גופן נקי (`Segoe UI`/Arial), הגדלת גודל גופן בסיסי (1.05rem), איסור מוחלט על italics (`font-style: normal !important`). **מגבלה ידועה:** מאחר ש-`st.dataframe` מצייר את הרשת הפנימית (`glide-data-grid`) על HTML canvas, CSS גלובלי לא יכול לעצב תא-תא בתוך הרשת עצמה (רק את המכולות סביבה) - תועד בקוד.
- [x] **TDD:** `test_apis.py`/`test_cross_ref.py`/`test_sdk.py` עודכנו לשדות המספריים החדשים; `test_table_rows.py` נכתב מחדש (כולל `recommended_vendor`); `test_table_controls.py` - הוסרה בדיקת מיון "מחיר ליחידה" הישנה (עמודת המחיר כעת מספרית וממוינת נכון באופן טבעי, ללא צורך בחילוץ Regex). אומת גם קצה-לקצה עם `streamlit.testing.v1.AppTest` מול תרחיש עם נתוני ספקים חלקיים (חלק null) - ללא חריגות, "ספק מומלץ" חושב נכון. 122 בדיקות עוברות, כיסוי כולל 94%.

## פריטים שנדחו במכוון מעבר ל-MVP (Out of Scope, לא חוסמים סגירת שלב הפיתוח)
- **כיסוי בדיקות מעבר ל-85%:** הכיסוי הכולל עומד על כ-94% (מעל סף ה-85% הנדרש), עם 122 בדיקות עוברות. `cross_ref.py`/`sdk.py`/`gatekeeper.py`/`cli/main.py` אינם מכוסים ב-100% (מסלולי שגיאת רשת/CLI קיצוניים) - שיפור אפשרי לשלב תחזוקה עתידי, לא חוסם.
- **כותרות מקובצות אמיתיות (spanning headers):** לא ניתנות למימוש ב-`st.dataframe` הנוכחי יחד עם `column_config` (ראו Phase 15) - אם בעתיד יהיה חשוב יותר להראות כותרת-על אמיתית מאשר עיצוב `NumberColumn`/`ProgressColumn`, ניתן לשקול חזרה ל-`pandas.Styler`/HTML, במחיר אובדן היתרונות של `st.dataframe` הטבעי.

---
## סטטוס פרויקט: שלב הפיתוח הושלם (Development Phase Complete)
כל שלבי הפיתוח שהוגדרו במסמך זה (Phase 1 עד Phase 15) הושלמו ואומתו בבדיקות, כולל שלב הליטוש הסופי, אינטגרציית DigiKey ו-Octopart המלאות (כולל null-safety מלא מול תגובות GraphQL אמיתיות), תיקון תקלת ה-Retry הקריטית, מיגרציה מלאה ומאומתת מ-`components.html` המיושן ל-`st.iframe`/`st.dataframe` בכל הקוד, שיפורי UX לפי היוריסטיקות של Nielsen, ושדרוג הטבלה המלא (השוואת שלושת הספקים side-by-side עם "ספק מומלץ", `st.dataframe` טבעי במקום iframe, RTL/טיפוגרפיה). המערכת כעת multi-provider אמיתית ומאומתת (Mouser + DigiKey + Octopart) כפי שתוכנן במקור ב-PRD סעיף 4.2, ומוכנה לשלב אריזת ה-Desktop הבא. אין פריטים פתוחים חוסמים; ההמשך (כיסוי 100%, כותרות מקובצות אמיתיות אם יידרש בעתיד) מנוהל כתחזוקה שוטפת, בכפוף לאותה מתודולוגיית עבודה (PRD → PLAN → TODO → קוד) המתוארת ב-`docs/CLAUDE.md`.