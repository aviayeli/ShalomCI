...
from src.gui.table_format import (latin_ltr, mpn_bidi, price_text, risk_color, stock_text,
                                  vendor_badge)
...
def test_price_text_formats_currency_and_handles_missing(value, expected):
    ...
    assert price_text(value) == expected
...
def test_stock_text_formats_thousands_and_handles_missing(value, expected):
    assert stock_text(value) == expected
...
def test_risk_color_maps_score_to_traffic_light(score, expected_css):
    assert risk_color(score) == expected_css
...
def test_mpn_bidi_escapes_malicious_content_inside_the_tag():
    ...
    result = mpn_bidi("<script>alert(1)</script>")
...
def test_latin_ltr_wraps_only_hebrew_free_values(value, expected):
    ...
    assert latin_ltr(value) == expected


def test_latin_ltr_escapes_malicious_content():
    """מוודא ש-latin_ltr מבריח HTML בשני המסלולים (עטוף ולא-עטוף) - הוא מחליף את
    escape="html" של ה-Styler בעמודות שהוא מפרמט."""
    assert latin_ltr("<script>x</script>") == '<bdo dir="ltr">&lt;script&gt;x&lt;/script&gt;</bdo>'
    assert latin_ltr("עברית <b>") == "עברית &lt;b&gt;"
...
def test_vendor_badge_escapes_malicious_content_inside_the_span():
    """מוודא שתוכן הספק מוברח נגד XSS בדיוק כמו ב-mpn_bidi (format ללא escape="html")."""
    result = vendor_badge("<script>alert(1)</script>")
    assert result == (
